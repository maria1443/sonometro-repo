...
const DevicePromDayHome = ({ promAlertByDay, alertPromAverage, deviceAlerts }) => {
  const trendDataMeasurementsAlerts = () => {
    if (promAlertByDay != undefined) {
      const createdMeasureAlerts = promAlertByDay.map((data) => data.created);
    ...
    <>
      ...
      <div className="flex mt-7" style={{ marginLeft: "15%" }}>
        ...
        >
          ...
          <p className="text-gray-800 ml-5">{alertPromAverage.avgValue}</p>
        </div>
        <div
          className="flex items-center justify-between bg-white p-3"
          style={{ marginLeft: "5%" }}
        >
          ...
          <p className="text-gray-800 ml-5">{alertPromAverage.stdValue}</p>
        </div>
      </div>
    </>
  );
};
export default DevicePromDayHome;
